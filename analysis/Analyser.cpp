      ...
      newFrameCallback([](){})
{
    ...
    qInfo() << oboe::getVersionText();
//    setInputDevice(Pa_GetDefaultInputDevice());
    qInfo() << "Default Input Device from within analyser " << audioDevices.defaultInputDeviceId;
    setInputDevice(audioDevices.defaultInputDeviceId);
}
